    ...
    IERC721Enumerable
{
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _beforeTokenTransfers(owner, address(0), tokenId);
        ...
        _afterTokenTransfers(owner, address(0), tokenId);
    }
...
contract DojojiBell is ERC721A, ERC5050, Ownable, ReentrancyGuard {
    ...
    function claimBell() external payable nonReentrant {
        ...
        require(
            ...
            unicode'Wow, pls attach 0.0222 ether per bell'
        );
    ...
    function sendPray() external payable {
        userfrom = Object({
            _address: msg.sender,
            _tokenId: originalRinger[msg.sender]
        });
    ...
    // 🔔
    function teamMinting(uint256 _bell) public onlyOwner {
        require(!teamMinted);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "We are Dojoji", "image": ',
                        url,
                        ',  "attributes":[{"trait_type":"Number of prayers","value":',
                        userTotalPrays[tokenId],
                        '},{"trait_type":"Number of Rings","value":',
                        userTotalRings[tokenId],
                        '},{"trait_type":"Is Enlightened?","value":',
                        _isEnlightened[tokenId],
                        '}}'
                    )
                )
            )
